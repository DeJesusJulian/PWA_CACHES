...
self.addEventListener('install', (event) => {
    ...
    const promesaCache = caches.open(CACHE_STATIC_NAME).then((cache) => {
        return cache.addAll([
            '/PWA_CACHES/',
            '/PWA_CACHES/index.html',
            '/PWA_CACHES/css/pages.css',
            '/PWA_CACHES/img/imagen.jpg',
            '/PWA_CACHES/js/app.js',
            '/PWA_CACHES/pages/viewOffline.html',
            '/PWA_CACHES/img/imagen2.jpg'
        ])
    })
            ...
            return fetch(event.request)
                .then(respNet => {
                    ...
                    if(event.request.headers.get('accept').includes('text/html')){
                        return caches.match('/PWA_CACHES/pages/viewOffline.html')
                    }
